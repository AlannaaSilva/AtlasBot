import { Search, Sparkles, Clock, ChevronRight, BookOpen, HelpCircle, Zap } from "lucide-react";
  ...
  return (
    <aside className="w-72 bg-sidebar p-6 flex flex-col gap-5 shrink-0 hidden md:flex border-r border-sidebar-border">
      ...
      <div className="flex items-center gap-3 px-2">
        ...
        <div>
          <span className="text-sm font-semibold text-sidebar-primary">Assistente</span>
      ...
      <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl bg-sidebar-accent/40 border border-sidebar-border hover:bg-sidebar-accent/60 transition-colors cursor-pointer group">
        <Search size={14} className="text-sidebar-primary group-hover:scale-110 transition-transform" />
      ...
      <nav className="space-y-1">
        ...
        >
          <HelpCircle size={14} className={activeCategory === null ? "text-sidebar-primary" : "text-sidebar-muted"} />
        ...
        {categories.map((cat) => (
          ...
          >
            ...
            <ChevronRight size={14} className="opacity-0 group-hover:opacity-60 transition-opacity text-sidebar-primary" />
          </button>
        ))}
      </nav>
      ...
      <div className="mt-auto p-4 rounded-xl bg-gradient-to-br from-sidebar-accent/50 to-sidebar-accent/30 border border-sidebar-border">
        <div className="flex items-start gap-2">
          <Sparkles size={14} className="text-sidebar-primary mt-0.5 shrink-0" />
          <div className="text-[10px] text-sidebar-muted leading-relaxed">
            <span className="text-sidebar-foreground/80 font-medium">RAG Ativo</span>
            <br />
